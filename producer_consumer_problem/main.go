...
// pizzaria is a goroutine that runs in the background and
...
func pizzaria(pizzaMaker *Producer) {
	...
	for {
		...
		if currentPizza != nil {
			...
			select {
			// we tried to make a pizza (we sent somthing to the data channel)
...
func main() {
	...
	// create & run a consumer
	for i := range pizzaJob.data {
		if i.pizzaNumber <= NumberOfPizzas {
			if i.success {
				color.Blue(i.message)
				color.Blue("Order #%d is out for delivery!", i.pizzaNumber)
			} else {
				color.Red(i.message)
